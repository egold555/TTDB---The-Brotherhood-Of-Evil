...
public class FileUtil {
	...
	public static void saveToFile(Object list, String filename) {
		...
		try {
			...
			AbstractTeenTitanBot.GSON.toJson(list, writer);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
	...
	public static <T> List<T> loadArrayFromFile(String filename, Class<T[]> clazz) {
		...
		try {
			...
			T[] arr = AbstractTeenTitanBot.GSON.fromJson(reader, clazz);
	...
	public static <T> T loadFromFile(String filename, Class<T> clazz) {
		...
		try {
			reader = new FileReader("res/" + filename + ".json");
			return AbstractTeenTitanBot.GSON.fromJson(reader, clazz);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
